...
- `skills/` is the product. No `commands/` directory — skills ARE the slash commands (frontmatter `name: actuals:<skill>` gives `/actuals:<skill>`).
...
- **Scripts:** Node 22, zero dependencies, ESM (.mjs). Each is a library (named exports) + CLI (`node script.mjs <args> --json`). System Python here is 3.9 — don't add Python.
